...
void window_init(bread_window_t *window) { get_backend_vtable()->init(window); }

void window_poll(bread_window_t *window) {
  get_backend_vtable()->poll_events(window);
}

b32 window_should_close(bread_window_t *window) {
  return get_backend_vtable()->should_close(window);
}

void window_destroy(bread_window_t *window) {
  get_backend_vtable()->destroy(window);
}

bread_surface_t window_get_surface(bread_window_t *window) {
  return get_backend_vtable()->get_surface(window);
}

bread_backend_type_t window_get_backend_type(bread_window_t *window) {
