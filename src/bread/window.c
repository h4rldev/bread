...
void bread_window_init(bread_window_t *window) {
  ...
  if (!backend_vtable) {
    bread_log_fatal("No backend vtable, can't init window");
    return;
  }
...
void bread_window_set_title(bread_window_t *window, const char *title) {
  ...
  const bread_backend_vtable_t *backend_vtable = get_backend_vtable();
  if (!backend_vtable) {
    bread_log_fatal("No backend vtable, can't set title");
    return;
  }

  bread_log_debug("Setting window title to '%s'", title);
  backend_vtable->set_title(window, title);
}
...
void bread_window_set_min_size(bread_window_t *window, u16 width, u16 height) {
  ...
  const bread_backend_vtable_t *backend_vtable = get_backend_vtable();
  if (!backend_vtable) {
    bread_log_fatal("No backend vtable, can't set min size");
    return;
  }

  bread_log_debug("Setting window min size to %ux%u", width, height);
  backend_vtable->set_min_size(window, width, height);
}
...
void bread_window_poll(bread_window_t *window) {
  ...
  const bread_backend_vtable_t *backend_vtable = get_backend_vtable();
  if (!backend_vtable) {
    bread_log_fatal("No backend vtable, can't poll");
    return;
  }
...
b32 bread_window_should_close(bread_window_t *window) {
  ...
  const bread_backend_vtable_t *backend_vtable = get_backend_vtable();
  if (!backend_vtable) {
    bread_log_fatal("No backend vtable, can't check if should close");
    return false;
  }

  return backend_vtable->should_close(window);
}
...
void bread_window_destroy(bread_window_t *window) {
  ...
  const bread_backend_vtable_t *backend_vtable = get_backend_vtable();
  if (!backend_vtable) {
    bread_log_fatal("No backend vtable, can't destroy window");
    return;
  }
...
bread_surface_t bread_window_get_surface(bread_window_t *window) {
  ...
  const bread_backend_vtable_t *backend_vtable = get_backend_vtable();
  if (!backend_vtable) {
    bread_log_fatal("No backend vtable, can't get surface");
    return (bread_surface_t){0};
  }

  bread_log_debug("Getting surface");
  return backend_vtable->get_surface(window);
}
...
bread_backend_type_t bread_get_backend_type(void) {
  ...
  const bread_backend_vtable_t *backend_vtable = get_backend_vtable();
  if (!backend_vtable) {
    bread_log_fatal("No backend vtable, can't get backend type");
    return BREAD_BACKEND_UNKNOWN;
  }
